...
class ConnectionListWidget(ttk.Frame):
  def __init__(self, master=None, **kwargs): 
    ...
    self.button = ttk.Button(self, text="update", command=lambda: self.update_data(dummy_connections)).grid(column=0, row=0)
  ...
  def update_data(self, data):
    ...
    for i, connection in enumerate(data):
      self.label = ttk.Label(self, text=f"{connection.name}" + str(self.number)).grid(column=0, row=i+2)
      self.label2 = ttk.Label(self, text=f"{connection.linkedin}").grid(column=1, row=i+2)